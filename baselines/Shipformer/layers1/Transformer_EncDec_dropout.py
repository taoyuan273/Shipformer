...
from baselines.Shipformer.utils.tools import hier_half_token_weight
from baselines.Shipformer.layers1.SelfAttention_Family import FullAttention, AttentionLayer
from baselines.Shipformer.layers1.Embed import PatchEmbedding

from baselines.Shipformer.utils.tools import create_sin_pos_embed
import math
import random
from typing import List
from baselines.Shipformer.layers1.RevIN import RevIN

from baselines.Shipformer.utils.CKA import CudaCKA
...
class EncoderLayer(nn.Module):
    def __init__(self, attention, d_model, d_ff=None, dropout=0.3, activation="relu", MLP_flag=True):
...
class Encoder_ori(nn.Module):
    def __init__(self, attn_layers, conv_layers=None, norm_layer=None, one_output=False, CKA_flag=False, dropout=0.3):
        super(Encoder_ori, self).__init__()
        self.attn_layers = nn.ModuleList(attn_layers)
        self.norm = norm_layer
        self.one_output = one_output
        self.CKA_flag = CKA_flag
        self.dropout = nn.Dropout(dropout)  # 🔥 这里加 Dropout
        if self.CKA_flag:
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None):
        ...
        if self.norm is not None:
            x = self.norm(x)
            x = self.dropout(x)  # 🔥 **在 LayerNorm 之后加 Dropout**
