...
from baselines.Shipformer.utils.masking import TriangularCausalMask, ProbMask
from reformer_pytorch import LSHSelfAttention
from einops import rearrange
import os
import random

from baselines.Shipformer.utils.tools import create_sub_diagonal_matrix, plot_mat
...
class FullAttention_ablation(nn.Module):
    def __init__(self, mask_flag=False, factor=5, scale=None, attention_dropout=0.3, output_attention=False,
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None, token_weight=None):
        ...
        scores = self.dropout(scores)  # ✅ 在 raw attention scores 上应用 Dropout
        ...
        if self.SF_mode and weight_mat is not None:
            ...
            A = torch.softmax(A, dim=-1)
        A = self.dropout(A)  # ✅ 在 softmax 之后的 attention matrix 上应用 Dropout
        ...
        # print(f'values.shape: {values.shape}')
        V = torch.einsum("bhls,bshd->blhd", A, values)
        V = self.dropout(V)  # ✅ 在最终的加权和值 V 上应用 Dropout
...
class AttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
        ...
        self.norm_q = nn.LayerNorm(16)
        self.norm_k = nn.LayerNorm(d_keys * n_heads)
        self.norm_v = nn.LayerNorm(d_values * n_heads)
