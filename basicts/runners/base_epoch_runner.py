    ...
    def train(self, cfg: Dict) -> None:
        ...
        for epoch_index in range(self.start_epoch, self.num_epochs):
            ...
            for iter_index, data in enumerate(data_loader):
                ...
                batch_x = data['inputs']  # 访问字典中的 'inputs' 键
                # 假设 batch_y 存储在另一个键下，像这样访问:
                batch_y = data['target']  # 如果标签存储在 'labels' 键下

                #print(f"batch_x shape: {batch_x.shape}")
                #print(f"batch_y shape: {batch_y.shape}")

                # 频域数据增强 - 给输入数据加噪声
                if self.freq_augmentation and epoch_index % 2 == 1:
                    perm = torch.randperm(batch_x.size(2))  # 随机排列通道
                    pseudo_channel = torch.normal(mean=0, std=self.sigma, size=(batch_x.size(0), batch_x.size(2))).unsqueeze(-2).unsqueeze(-1)
                    batch_x = batch_x + batch_x[:, :, perm] * pseudo_channel  # 给输入数据加噪声

                # 频域数据增强 - 给标签数据加噪声
                if self.freq_augmentation and epoch_index % 2 == 1:
                    perm = torch.randperm(batch_y.size(2))  # 随机排列通道
                    pseudo_channel = torch.normal(mean=0, std=self.sigma, size=(batch_y.size(0), batch_y.size(2))).unsqueeze(-2).unsqueeze(-1)
                    batch_y = batch_y + batch_y[:, :, perm] * pseudo_channel  # 给目标数据加噪声
